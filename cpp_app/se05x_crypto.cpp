...
RsaKey RsaKey::generate(Session &s, uint32_t keyId, RsaBits bits, sss_policy_t *policy) {
    ...
    check(sss_key_object_allocate_handle(&k.obj_, keyId, kSSS_KeyPart_Pair, kSSS_CipherType_RSA,
                                         k.bits_ / 8, kKeyObject_Mode_Persistent),
          "key_object_allocate_handle(RSA)");
    check(sss_key_store_generate_key(s.keystore(), &k.obj_, static_cast<uint32_t>(k.bits_), policy),
...
RsaKey::RsaKey(RsaKey &&o) noexcept : s_(o.s_), obj_(o.obj_), bits_(o.bits_), owns_(o.owns_) {
    o.owns_ = false;
}
