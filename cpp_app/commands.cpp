...
se05x::Session &CommandContext::requireMgmt() const {
    if (!mgmt)
        throw std::runtime_error(
            "this command needs a direct SE session; omit --pkcs11 and set "
...
std::string bytesToDisplay(const std::vector<uint8_t> &b) {
    ...
    for (uint8_t c : b)
        if (!(c == '\t' || c == '\n' || c == '\r' || (c >= 0x20 && c < 0x7f))) { printable = false; break; }
    if (printable) return std::string(b.begin(), b.end());
    std::string hex;
    char        tmp[4];
    for (uint8_t c : b) { std::snprintf(tmp, sizeof(tmp), "%02x", c); hex += tmp; }
    return hex;
}
...
public:
    ...
    const char *help() const override { return "rng <nbytes>            random bytes from the SE TRNG"; }
...
public:
    ...
    const char *help() const override { return "se  uid                 read the 18-byte chip UID"; }
...
public:
    ...
    const char *help() const override {
        return "se  rotate-scp03        --enc/--mac/--dek <hex32> [--dry-run|--confirm] [--key-out <f>]";
    }
    ...
    int run(CommandContext &ctx, const Args &a) const override {
        ...
        auto enc = parseKey16(a, "--enc"), mac = parseKey16(a, "--mac"), dek = parseKey16(a, "--dek");
        ...
        if (outPath.empty()) {
            ...
            Log::get().print("ENC %s\nMAC %s\nDEK %s\n",
                             a.get("--enc").c_str(), a.get("--mac").c_str(), a.get("--dek").c_str());
            return 1;
        }
        try {
...
public:
    ...
    const char *help() const override {
        return "se  write-info         (--data <text> | --data-hex <hex>) [--id <hex>=0xF0000010] [--force]";
    }
    SessionNeed sessionNeed() const override { return SessionNeed::Management; }
    int run(CommandContext &ctx, const Args &a) const override {
...
public:
    ...
    const char *help() const override { return "se  read-info          [--id <hex>=0xF0000010] [--out <f>]"; }
    SessionNeed sessionNeed() const override { return SessionNeed::Management; }
    int run(CommandContext &ctx, const Args &a) const override {
...
public:
    ...
    const char *help() const override {
        return "se  verify-info        (--data <text> | --data-hex <hex>) [--id <hex>=0xF0000010]";
    }
    SessionNeed sessionNeed() const override { return SessionNeed::Management; }
    int run(CommandContext &ctx, const Args &a) const override {
        const uint32_t id       = parseId(a, kDeviceInfoId);
...
public:
    ...
    const char *help() const override {
        return "rsa genkey             [--id <hex>] [--bits 2048|3072|4096] [--policy ...] [--force] [--out <f>]";
    }
    SessionNeed sessionNeed() const override { return SessionNeed::Crypto; }
    int run(CommandContext &ctx, const Args &a) const override {
        ...
        if (ctx.crypto.keyExists(id)) {
            if (!a.flag("--force")) {
                ...
                if (!a.get("--out").empty()) ctx.out.spki(a, ctx.crypto.getSpki(id));
                return 0;
            }
...
public:
    ...
    const char *help() const override { return "rsa pub                [--id <hex>] [--out <f>] [--pem]"; }
    SessionNeed sessionNeed() const override { return SessionNeed::Crypto; }
    int run(CommandContext &ctx, const Args &a) const override {
...
public:
    ...
    const char *help() const override { return "rsa sign               [--id <hex>] --in <f> [--out <f>]"; }
    SessionNeed sessionNeed() const override { return SessionNeed::Crypto; }
    int run(CommandContext &ctx, const Args &a) const override {
        ctx.out.bytes(a, ctx.crypto.sign(parseId(a, kRsaKeyId), OutputWriter::readFile(a.get("--in"))));
        return 0;
    }
};
...
public:
    ...
    const char *help() const override { return "rsa verify             [--id <hex>] --in <f> --sig <f>"; }
    SessionNeed sessionNeed() const override { return SessionNeed::Crypto; }
    int run(CommandContext &ctx, const Args &a) const override {
        bool ok = ctx.crypto.verify(parseId(a, kRsaKeyId),
                                    OutputWriter::readFile(a.get("--in")),
...
public:
    ...
    const char *help() const override { return "rsa csr                [--id <hex>] --subject \"CN=...\" [--out <f>]"; }
    SessionNeed sessionNeed() const override { return SessionNeed::Crypto; }
    int run(CommandContext &ctx, const Args &a) const override {
...
public:
    ...
    const char *help() const override { return "rsa write-cert         [--id <hex>] --in <cert.der>   (SSS only)"; }
    SessionNeed sessionNeed() const override { return SessionNeed::Management; }
    int run(CommandContext &ctx, const Args &a) const override {
...
public:
    ...
    const char *help() const override {
        return "rsa verify-binding     [--id <hex>=key] (--cert <cert.der> | --cert-id <hex>=0xF0000002) (SSS only)";
    }
    SessionNeed sessionNeed() const override { return SessionNeed::Management; }
    int run(CommandContext &ctx, const Args &a) const override {
...
const Command *CommandRegistry::find(const std::string &group, const std::string &command) const {
    ...
    if (def != index_.end()) return def->second;
    return nullptr;
}
