...
Pkcs11Error::Pkcs11Error(const std::string &msg, CK_RV rv)
    : std::runtime_error([&] {
          char buf[64];
          std::snprintf(buf, sizeof(buf), ": CKR 0x%08lX",
...
static void ckCheck(CK_RV rv, const char *where) {
    if (rv != CKR_OK) throw Pkcs11Error(std::string("PKCS#11 ") + where + " failed", rv);
}

Pkcs11Ctx::Pkcs11Ctx(const std::string &libPath) {
    lib_ = dlopen(libPath.c_str(), RTLD_NOW);
    if (!lib_) throw Pkcs11Error(std::string("dlopen(") + libPath + "): " + dlerror());

    auto getFnList =
        reinterpret_cast<CK_C_GetFunctionList>(dlsym(lib_, "C_GetFunctionList"));
    if (!getFnList) {
        dlclose(lib_);
        lib_ = nullptr;
    ...
    ckCheck(p11_->C_OpenSession(slots[0], CKF_SERIAL_SESSION | CKF_RW_SESSION, nullptr,
                                nullptr, &hSess_),
            "C_OpenSession");
}
...
std::pair<CK_OBJECT_HANDLE, CK_OBJECT_HANDLE> Pkcs11Ctx::genRsaKeyPair(uint32_t id,
                                                                       CK_ULONG bits) {
    auto                 lbl       = label(id);
    auto                 idb       = idBytes(id);
    ...
    ckCheck(p11_->C_GenerateKeyPair(
                hSess_, &mechKG, pubTmpl, sizeof(pubTmpl) / sizeof(pubTmpl[0]), privTmpl,
                sizeof(privTmpl) / sizeof(privTmpl[0]), &hPub, &hPriv),
...
CK_OBJECT_HANDLE Pkcs11Ctx::findKey(uint32_t id, CK_OBJECT_CLASS cls) {
    ...
    if (p11_->C_FindObjectsInit(hSess_, tmpl, 2) != CKR_OK) return CK_INVALID_HANDLE;
...
void Pkcs11Ctx::destroyObject(CK_OBJECT_HANDLE h) {
    if (h != CK_INVALID_HANDLE) p11_->C_DestroyObject(hSess_, h);
}

std::vector<uint8_t> Pkcs11Ctx::signRsa(CK_OBJECT_HANDLE            hPriv,
...
bool Pkcs11Ctx::verifyRsa(CK_OBJECT_HANDLE hPub, const std::vector<uint8_t> &msg,
    ...
    CK_MECHANISM mech = {CKM_SHA256_RSA_PKCS, nullptr, 0};
    if (p11_->C_VerifyInit(hSess_, &mech, hPub) != CKR_OK) return false;
    CK_RV rv = p11_->C_Verify(
        hSess_, const_cast<CK_BYTE *>(msg.data()), static_cast<CK_ULONG>(msg.size()),
...
std::vector<uint8_t> Pkcs11Ctx::encryptRsa(CK_OBJECT_HANDLE            hPub,
    ...
    CK_RSA_PKCS_OAEP_PARAMS params{CKM_SHA256, CKG_MGF1_SHA256, CKZ_DATA_SPECIFIED,
                                   nullptr, 0};
...
std::vector<uint8_t> Pkcs11Ctx::getSpki(uint32_t id) {
    ...
    auto readAttr = [&](CK_ATTRIBUTE_TYPE type) {
        ...
        if (rv != CKR_OK) throw Pkcs11Error("C_GetAttributeValue (value) failed", rv);
        return val;
    };
    ...
    mbedtls_pk_free(&pk);
    if (n <= 0) throw Pkcs11Error("mbedtls_pk_write_pubkey_der failed");
