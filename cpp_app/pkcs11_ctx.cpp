...
Pkcs11Ctx::Pkcs11Ctx(const std::string &libPath) {
    ...
    ckCheck(p11_->C_OpenSession(slots[0],
                                CKF_SERIAL_SESSION | CKF_RW_SESSION,
                                nullptr, nullptr, &hSess_),
            "C_OpenSession");
}
...
Pkcs11Ctx::genRsaKeyPair(uint32_t id, CK_ULONG bits) {
    auto lbl = label(id);
    auto idb = idBytes(id);
...
std::vector<uint8_t> Pkcs11Ctx::signRsa(CK_OBJECT_HANDLE hPriv,
    ...
    CK_ULONG sigLen = 0;
    ckCheck(p11_->C_Sign(hSess_,
                         const_cast<CK_BYTE *>(msg.data()),
                         static_cast<CK_ULONG>(msg.size()),
                         nullptr, &sigLen),
            "C_Sign(RSA, query)");
...
bool Pkcs11Ctx::verifyRsa(CK_OBJECT_HANDLE hPub,
                           const std::vector<uint8_t> &msg,
                           const std::vector<uint8_t> &sig) {
    CK_MECHANISM mech = { CKM_SHA256_RSA_PKCS, nullptr, 0 };
    if (p11_->C_VerifyInit(hSess_, &mech, hPub) != CKR_OK) return false;
    CK_RV rv = p11_->C_Verify(hSess_,
                               const_cast<CK_BYTE *>(msg.data()),
                               static_cast<CK_ULONG>(msg.size()),
                               const_cast<CK_BYTE *>(sig.data()),
                               static_cast<CK_ULONG>(sig.size()));
    return (rv == CKR_OK);
}
...
std::vector<uint8_t> Pkcs11Ctx::encryptRsa(CK_OBJECT_HANDLE hPub,
    ...
    CK_ULONG outLen = 0;
    ckCheck(p11_->C_Encrypt(hSess_,
                             const_cast<CK_BYTE *>(plain.data()),
...
std::vector<uint8_t> Pkcs11Ctx::decryptRsa(CK_OBJECT_HANDLE hPriv,
    ...
    CK_ULONG outLen = 0;
    ckCheck(p11_->C_Decrypt(hSess_,
                             const_cast<CK_BYTE *>(cipher.data()),
                             static_cast<CK_ULONG>(cipher.size()),
                             nullptr, &outLen),
            "C_Decrypt (query)");
    std::vector<uint8_t> out(outLen);
    ckCheck(p11_->C_Decrypt(hSess_,
...
std::vector<uint8_t> Pkcs11Ctx::getRandom(size_t n) {
    ...
    ckCheck(p11_->C_GenerateRandom(hSess_, out.data(),
                                   static_cast<CK_ULONG>(n)),
            "C_GenerateRandom");
    return out;
}
